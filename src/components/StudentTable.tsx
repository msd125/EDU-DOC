        ...
        >
          <thead className="text-[10px] sm:text-xs md:text-sm uppercase sticky top-0 z-30"
            ...
            <tr>
              ...
              {columns.map((col) => (
                <th key={col.id} className="px-1 sm:px-2 py-2 min-w-[80px] md:min-w-[120px] max-w-[180px] text-center align-top border-slate-200 border-l last:border-l-0 group relative text-xs font-bold"
                    style={{ fontFamily: 'Noto Sans Arabic, Cairo, sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: getContrastColor(themeColor) }}>
                    ...
                    {col.type === 'قائمة' || col.type === 'LIST' ? (
                      ...
                      <div className="flex gap-1 mt-1 justify-center items-center">
                        ...
                        <button
                          ...
                          title="تفريغ الكل"
                          onClick={() => onFillColumn && onFillColumn(col.id, null)}
                        >تفريغ</button>
                      </div>
                    ) : (
